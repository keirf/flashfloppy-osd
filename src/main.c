...
static time_t this_sync_time, last_sync_time, avg_sync_time;
...
//      printk("avg: %d\n", avg_sync_time);
        ...
        for (int i=0; i<sync_log_MAX; i++) {
            if ( (sync_log[i] - avg_sync_time) > 10 )
                valid_sync_data = FALSE;
            if ( (avg_sync_time - avg_sync_time) > 10 )
                valid_sync_data = FALSE;
        }
        ...
        if (valid_sync_data) {
            /* sync time -> sync frequency in Hz */
            avg_sync_time = 9000000 / avg_sync_time;
            if ((avg_sync_time < 20000) && (config.display_timing != DISP_15KHZ)) {
                /* PAL/NTSC */
                config.display_timing = DISP_15KHZ;
                config.polarity = FALSE;
                printk("switch to pal/ntsc. %d Hz < 20kHz\n",avg_sync_time);
                setup_spi();
            } else if ((avg_sync_time >= 20000) && (config.display_timing != DISP_VGA)) {
                /* VGA */
                config.display_timing = DISP_VGA;
                config.polarity = TRUE;
